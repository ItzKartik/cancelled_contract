    ...
    web3.eth.getAccounts().then(function (accounts) {
        ...
        if (val === 'stake') {
            ...
            m_Con.methods.balanceOf(acc).call().then(function (tx) {
                $('#staked_amt_2').html(tx);
            }).catch(function (tx) {
                console.log(tx);
            });

            m_Con.methods.userRewards(acc).call().then(function (tx) {
                $('#unclaimed_reward_2').html(tx['rewards']);
                $('.dragon_val_2').html(tx['rewards']);
            }).catch(function (tx) {
                console.log(tx);
            });
